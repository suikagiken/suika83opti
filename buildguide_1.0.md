...
### Remapを使う場合
1. [こちらのRemap定義ファイル](definition_remap/suika83opti.json) をダウンロードし、適当な場所に保存する
2. Chrome または Edge ブラウザで https://remap-keys.app/ にアクセスする
3. 「キーボードをカスタマイズ」ボタンをクリックする
4. 「+ キーボード」をクリックすると、「remap-keys.appがHIDデバイスへの接続を要求しています」という画面が開くので「suika83opti」を一覧から選択し「接続」をクリックする
